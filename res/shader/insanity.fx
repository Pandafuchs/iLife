...
float4 PixelShaderInsanity(float2 TextureCoordinates : TEXCOORD) : COLOR
{
	...
	float2 coordsDelta = {sin(Time + distortCoords.x * wobblingStrength + distortCoords.y * distortCoords.y * wobblingStrength) * 0.01 , cos(Time + distortCoords.y + distortCoords.x * wobblingStrength * distortCoords.x) * 0.02};
